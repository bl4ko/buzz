...
mod tests {
    ...
    async fn w8_membership_check_barrier_fires_before_db_read() {
        ...
        let state = audio_test_state().await;
        // Fresh UUID — isolates audio_membership_check_hook slot from any other
        // concurrent test that could drive before_membership_check with the same
        // key when both run in the same libtest process (fallback mode).
