...
`POST /v1/deliveries/apns` uses the public URL `https://push.buzz.xyz/v1/deliveries/apns`
or an explicitly operator-configured internal HTTP URL protected by authenticated
mesh transport. The relay MUST sign the same URL it sends the request to. The
gateway MUST verify the URL against the received request, including authority,
port, path and query; it MUST NOT obtain the expected URL from the signed event.
TLS-terminating ingress must preserve the authority and supply the original
HTTPS scheme to the gateway. Request:

```json
{"v":1,"endpoint_grant":"<opaque-capability>","request_id":"<uuid>","expires_at":<unix-seconds>}
```

The relay supplies a NIP-98 `Authorization: Nostr <standard-base64-event-json>` header for method `POST`, the exact request URL, and the SHA-256 payload hash of the **received request body bytes**. The gateway verifies the NIP-98 event signature, timestamp under NIP-98 rules, method, URL, and payload; the event pubkey is the relay identity. It decrypts `endpoint_grant`, requires that signer, current installation/delegation, endpoint epoch and generation, and both `now <= request.expires_at <= grant.expires_at`. Every NIP-98 event id is burned at admission.
